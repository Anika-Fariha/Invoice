...
export default function InvoiceFormPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = window.localStorage.getItem('companyInfo');
      if (saved) {
        ...
        setReadOnlyFields([
          'businessName', 'businessAddress', 'cityZip', 'taxId', 'website', 'phone', 'email'
        ]);
      }
    }
  }, []);
  ...
  return (
    <main className=' w-full h-full overflow-auto bg-slate-950 py-10 rounded-lg'>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input className="w-full border p-2 rounded" placeholder="Business Address" value={form.businessAddress} onChange={(e) => setForm({ ...form, businessAddress: e.target.value })} readOnly={isReadOnly('businessAddress')} />
            {errors.businessAddress && <p className="text-red-500 text-sm">{errors.businessAddress}</p>}

            <input className="w-full border p-2 rounded" placeholder="City, Zip" value={form.cityZip} onChange={(e) => setForm({ ...form, cityZip: e.target.value })} readOnly={isReadOnly('cityZip')} />
            {errors.cityZip && <p className="text-red-500 text-sm">{errors.cityZip}</p>}

            <input className="w-full border p-2 rounded" placeholder="Tax ID (optional)" value={form.taxId} onChange={(e) => setForm({ ...form, taxId: e.target.value })} readOnly={isReadOnly('taxId')} />
            ...
            <input className="border p-2 rounded" placeholder="Website" value={form.website} onChange={(e) => setForm({ ...form, website: e.target.value })} readOnly={isReadOnly('website')} />
            <input className="border p-2 rounded" placeholder="Phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} readOnly={isReadOnly('phone')} />
            <input className="border p-2 rounded" placeholder="Email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} readOnly={isReadOnly('email')} />
            </div>
